...
-- INSERT 의 두번째 방법(직접컬럼을 지정하고 저장, NOT NULL 컬럼을 확인)
INSERT INTO departments
...
-- WHERE 절에 false값(1 = 2) 지정하면 -> 테이블 구조만 복사되고 데이터는 복사x
-- CTAS 문은 컬럼의 구조와 데이터를 복사할 뿐 , 재약조건(PK, FK...)은 복제되지 않음
...
-- UPDATE를 진행 할때는 누구를 수정할 지 조건으로 잘 지목해야 합니다.
-- 그렇지 않으면 수정 대상이 전체 테이블로 지정 됩니다.
