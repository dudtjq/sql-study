...
-- 서식문자와 함께 사용하고 싶은 문자를 ""로 감싸서 전달
SELECT 
    TO_CHAR(sysdate, 'YYYY"년" MM"월" DD"일"') 
FROM dual;

-- 숫자를 문자로 TO_CHAR(값, 형식)
-- 형식에서 사용하는 '9'는 실제 숫자 9가 아니라 자리수를 표현하기 위한 기호임
...
-- 문자를 날짜로 변환 하는 함수 TO_DATE(값, 형식)
SELECT TO_DATE('2023-04-13') FROM dual;
-- 주어진 문자열 모두 변환해야 함, 일부만 변환할 수 없음
SELECT TO_DATE('2021-03-31 12:23:50', 'YY-MM-DD HH24:MI:SS') FROM dual;

SELECT TO_DATE('2024년02월12일', 'YYYY"년"MM"월"DD"일"') FROM dual;

-- xxxx년 xx월 xx일 -> 문자열 형식으로 변환하기
-- 조회 컬럼명은 dateInfo임
...
-- NULL 형태를 변환하는 함수NVL2(컬럼, NULL이 아닐 경우의 값, NULL 일 경우의 값)
SELECT 
 first_name,
 salary,
 NVL2(
